var _ = require('lodash'),
     ...
     */
    csvAutoParse = function (value) {
        if (value.charAt(0) === '"' && value.charAt(value.length - 1) === '"') {
            // remove surrounding double quotes and skip further parsing
                ...
                async.waterfall([
                    ...
                    (json, cb) => {
                        ...
                        parseCsv(data, {
                            columns: true,
                            escape: '\\',
                            auto_parse: csvAutoParse,
                            trim: true,
                            quote: true // avoid trimming surrounding quotes
                        }, cb);
                    }
                ], (err, parsed) => {
                    if (err) {
